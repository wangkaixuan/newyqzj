		...
		<div class="center border_box">
			<div class="built_top">
				<div class="classification clearfix_workbench border_box">
					<div class="nav_button border_box" :class="{hover:dataParameter.type == '2'}" @click="shwoDate(2)">指给我的<span class="total_num">6</span></div>
					<div class="nav_button border_box mar_left" :class="{hover:dataParameter.type == '1'}" @click="shwoDate(1)">指给单位<span class="total_num">226</span></div>
				</div>
				<div class="workbench_title border_box">
					<a href="javascript:void(0)"  class="border_box" :class="{hover:dataParameter.status == '0'}" @click="setNav('0')">待审批<span class="num" v-if="statusCount.unapprovedNum > 0">{{statusCount.unapprovedNum}}</span></a>
					<a href="javascript:void(0)"  class="border_box" :class="{hover:dataParameter.status == '3'}" @click="setNav('3')">待回执<span class="num" v-if="statusCount.receiptNum > 0">{{statusCount.receiptNum}}</span></a>
					<a href="javascript:void(0)"  class="border_box" :class="{hover:dataParameter.status == '1'}" @click="setNav('1')">通过<span class="num" v-if="statusCount.passedNum > 0">{{statusCount.passedNum}}</span></a>
					<a href="javascript:void(0)"  class="border_box" :class="{hover:dataParameter.status == '2'}" @click="setNav('2')">未通过<span class="num" v-if="statusCount.unpassedNum > 0">{{statusCount.unpassedNum}}</span></a>
					<a href="javascript:void(0)"  class="border_box" :class="{hover:dataParameter.status == '4'}" @click="setNav('4')">已完成<span class="num" v-if="statusCount.completedNum > 0">{{statusCount.completedNum}}</span></a>
				</div>
				<div class="search border_box">
					<ul class="clearfix_workbench">
						<li class="s_time border_box" :class="{hover:searchState == '0'}" @click.stop="searchTime('0')">
			              ...
			              <span class="hover">{{timeWz}}</span><i class="arrow"></i><span class="line">|</span>
			            </li>
			            ...
			            <li class="s_time border_box" :class="{hover:searchState == '3'}" @click.stop="searchTime('3')" v-if="status == '0'">
			            	<span class="work_date_title">审批类型</span><span class="hover">{{typeWz}}</span><i class="arrow"></i>
			            </li>
					...
					<div class="list_pub clearfix_workbench" v-for="list in listData">
						...
						<div class="info clearfix_workbench">
							<div class="info_body">
								...
								<div class="jb_info"> <!-- taskType : 1 上报 2 任务-->
									<p v-if="list.taskType == 2"><span>发起时间</span> {{list.createTime}}</p>
									<p v-else><span>上报时间</span> {{list.createTime}}</p>
									<!--<p><span>涉事地域</span> 北京-海淀</p>-->
									<p v-if="list.taskType == 2"><span>发起人</span> {{list.createUser}}</p>
									<p v-else><span>报送人</span> {{list.createUser}}</p>
								</div>
							</div>
							<!--操作-->
							<div class="info_cz">
...
import {getReviewListData, getStatusCount, getAllTypeData} from '../../service/built'
	...
	methods: {
	    ...
	    getStatusCountData(){
	        ...
	        let data = {
	          groupId: this.$store.state.generalGroupId,          //集团ID $store.state.generalGroupId
	          orgId: this.$store.state.orgId,                     //机构ID $store.state.orgId
	          type: this.dataParameter.type,                      // 1指给部门， 2指给个人
	        };
	        //获取舆情报送
	            ...
	            if(res.data.result){
	            	...
	            	if(result.length > 0){
		              	for(let i in result){
		                	...
		                	}else if(result[i].status == '3' || result[i].status == 2){
		                		_this.statusCount.receiptNum = result[i].count;
		                	}else if(result[i].status == '4' || result[i].status == 2){
		                		_this.statusCount.completedNum = result[i].count;
		                	}
		              	}
	              	}
	            }
