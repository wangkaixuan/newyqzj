		...
		<div class="center border_box">
			...
			<div class="new_built_content">
                <div class="new_built_list">
                    <label>标题</label><input type="text" name="built_title" class="title" v-model.trim="form.title"/>
                </div>
                ...
                <div class="new_built_list">
                    <label>来源网站</label><input type="text" name="net_source" class="net_source" v-model.trim="form.sourceWebsite"/>
                </div>
                ...
                <div class="new_built_list">
                    <label>处置建议</label>
                    <textarea class="txt_area" name="suggestions" v-model.trim="form.suggestions"></textarea>
                    ...
                    <div class="img_warys">
                        <div class="video_img">
                            <a href="javascript:void(0);" >
                                <em>+</em>选择图片
                                <form>
                                    <input type="file" accept="image/gif, image/jpeg, image/x-png, image/bmp" multiple="multiple" ref="built_img"  @change="uploadPic">
                                </form>
                            </a>
                        </div>
                ...
                <div class="new_built_list clearfix_new">
                    <div class="pos_left">
                        ...
                        <span class="add_icon">+
                            <form>
                             <input type="file" action-type="changeFile" node-type="file1" ref="built_file"  @change="uploadFile">
                            </form>
                        </span>
                    </div>
                    <div class="show_options">
                        <span class="enclosure" v-if="fileshowData.length > 0" v-for="fd,i in fileshowData">{{fd.name}}<span class="close" @click="delfile(i)">&times;</span></span>
                    </div>
                </div>
                ...
                <div class="new_built_list clearfix_new">
                    ...
                    <div class="show_options">
                        <span class="enclosure" v-if="instructionsAreData.length > 0" v-for="iad in instructionsAreData"><span name="instruct_user">{{iad.name}}</span><span class="close" v-on:click="closeAreData(iad.id)">&times;</span></span>
                    </div>
                </div>
                  ...
                  width="450px">
                    <div class="instructions_wary_box">
                        <div class="instructions_title_box">
                            <span v-bind:class="{hover:saveType==1}" @click="switchNav(1)">报给单位</span><span v-bind:class="{hover:saveType==2}" @click="switchNav(2)">报给领导</span>
                        </div>
                ...
                <!-- 按钮 -->
                <div class="built_btn_box">
                    <button type="button" class="btn built_submit" @click="submitBulit">提交</button>
                </div>
...
import {getAllTypeData,getInstructOtherTreeData,getInstructMyTreeData,saveBuiltSubmitData,getRestatementBuiltData} from '../../service/built'
...
export default {
	...
	methods: {
        ...
        uploadPic(){
            ...
            if(files){
                ...
                for (let i = 0, len = files.length; i < len; i++) {
                    ...
                    console.log('------排查重报-------');
                    console.log(_this.imagesfileData);
                    ...
                    if(window.FileReader){
                        ...
                        reader.onload = function (evt) {
                            _this.imagesfileData[i].src = evt.target.result
                            // console.log(i);
        ...
        delImags(s){   
            ...
            console.log('------删除后图片--------');
            console.log(this.imagesfileData);
        },
        //删除附件
        delfile(s){
        ...
        getThreeData(){
            ...
            getAllTypeData(parms).then(function (res) {
                console.log('---------获取处置建议、媒体类型、舆情分类列表-------------');
                console.log(res);
