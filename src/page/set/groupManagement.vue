<template>
	<div class="wrap">
		<yqzj-Head></yqzj-Head>
		<div class="center">
			<set-nav :hover="'organization'"></set-nav>
        ...
        <!-- :before-close="handleClose" -->
				<el-dialog
				  ...
				  width="360px"
          :show-close="false">
          <div class="group_sucess_tips">
            <!-- <p class="success_tips">提示</p> -->
            <p class="success_info">添加成功</p>
            <p class="success_info2">马上给新单位进行系统授权吧？</p>
          </div>
				...
				:close-on-click-modal=false  custom-class="add_group" @close="closeDialog" width="560px">
				  <el-form :model="form" ref="form" :rules="rules" :label-position="labelPosition" :label-width="formLabelWidth">
				    ...
				    <el-form-item label="使用人数" prop="maxUser">
				      <el-input-number v-model="form.maxUser" controls-position="right" @change="handleChange" :min="0" :max="10" label="描述文字"></el-input-number>
				    </el-form-item>
...
import {getGroupTree,checkSetMealUse,addGroupInfo} from '../../service/orgment'
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          // alert('submit!');
          if(that.delivery){
            that.form.isDatanode = 1;
          }else{
            that.form.isDatanode = 0;
          }

          that.form.groupId = parseInt(VueCookies.get('generalGroupId'));
          that.form.orgLevel = that.form.orgLevel +1;

          console.log('------------that.form2------------');
          console.log(that.form);

          addGroupInfo(that.form).then(function (res) {
            console.log('------------res------------');
            console.log(res);
            if(res.data.status == '0'){
              if(res.data.result.data.result == 0){  // 0 失败  1 成功
                that.dialogFormVisible = false;
                alert('保存失败');
              }
              if(res.data.result.data.result == 1){  // 0 失败  1 成功
                that.dialogFormVisible = false;
                alert('保存成功');
              }
            }
    ...
    addGroup: function (model) {
      ...
      let id = model.id, //id作为新增元素的pId
          orgLevel = model.orgLevel, //组织层级
          isDatanode = model.isDatanode; //是否是数据节点   0否 1 是

      this.form.pId = id;
      this.form.orgLevel = orgLevel;
      this.form.isDatanode = isDatanode;
    ...
    traversezTreeData:function (data) {
      ...
      for(let i in data){
        ...
        //console.log(parentData);
        parentData.children = [];
        parentData.ico = true; //是否显示增删改查icon
